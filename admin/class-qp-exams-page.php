...
class QP_Exams_Page {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['exam_id']) && check_admin_referer('qp_delete_exam_' . absint($_GET['exam_id']))) {
        ...
        // The query now correctly checks the qp_question_groups table
        $usage_count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}qp_question_groups WHERE exam_id = %d", $exam_id));
        
        if ($usage_count > 0) {
            // Updated error message for clarity
            QP_Sources_Page::set_message("This exam cannot be deleted because it is in use by {$usage_count} question group(s).", 'error');
        } else {
    ...
    // --- CORRECTED QUERY: Removed "exam_year" from ORDER BY clause ---
    $exams = $wpdb->get_results("SELECT * FROM $exams_table ORDER BY exam_name ASC");

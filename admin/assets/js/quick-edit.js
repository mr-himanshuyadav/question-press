jQuery(document).ready(function($) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $editRow.find(".inline-edit-col").html('<p style="color:red;">Error: Could not load the editor. ' + (response.data.message || "") + '</p>');
                }
            },
            error: function() {
                $editRow.find(".inline-edit-col").html('<p style="color:red;">Error: A server error occurred.</p>');
            }
        });
    });
    ...
    wrapper.on("change", ".qe-source-select", function() {
        ...
        // Update Sections dropdown based on the selected Source
        updateDropdown(
            $sectionSelect,
    ...
    wrapper.on('click', '.save', function(e) {
        ...
        var questionStatus = $('#post-' + questionId).find('.editinline').data('status');
        ...
        $.ajax({
            ...
            data: $formWrapper.find(':input').serialize() + '&' + $.param({
                action: 'save_quick_edit_data', // This AJAX action needs to be handled on the server
                question_id: questionId,
            ...
            success: function(response) {
                if (response.success && response.data.row_html) {
                    ...
                    // Hide the form and replace the original row with the updated HTML
    ...
    (function() {
        ...
        function manageBulkEditPanel() {
            ...
            if (currentSubject && currentSubject === initialSubject) {
                ...
                var availableTopics = qp_bulk_edit_data.topics.filter(t => t.subject_id == currentSubject);
                var linkedSourceIds = qp_bulk_edit_data.source_subject_links.filter(link => link.subject_id == currentSubject).map(link => link.source_id);
                var availableSources = qp_bulk_edit_data.sources.filter(source => linkedSourceIds.includes(source.source_id));
                var linkedExamIds = qp_bulk_edit_data.exam_subject_links.filter(l => l.subject_id == currentSubject).map(l => l.exam_id);
                var availableExams = qp_bulk_edit_data.exams.filter(e => linkedExamIds.includes(e.exam_id));

                $topicBulkEdit.html('<option value="">— Change Topic —</option>').prop("disabled", availableTopics.length === 0);
                $.each(availableTopics, (i, topic) => $topicBulkEdit.append($("<option></option>").val(topic.topic_id).text(topic.topic_name)));

                $sourceBulkEdit.html('<option value="">— Change Source —</option>').prop("disabled", availableSources.length === 0);
                $.each(availableSources, (i, source) => $sourceBulkEdit.append($("<option></option>").val(source.source_id).text(source.source_name)));

                $examBulkEdit.html('<option value="">— Change Exam —</option>').prop("disabled", availableExams.length === 0);
                $.each(availableExams, (i, exam) => $examBulkEdit.append($("<option></option>").val(exam.exam_id).text(exam.exam_name)));
        ...
        $bulkEditPanel.on("change", "#bulk_edit_source", function() {
            ...
            var $sectionBulkEdit = $("#bulk_edit_section");
            var availableSections = qp_bulk_edit_data.sections.filter(sec => sec.source_id == selectedSource);
            $sectionBulkEdit.html('<option value="">— Change Section —</option>').prop("disabled", availableSections.length === 0);
            $.each(availableSections, (i, section) => $sectionBulkEdit.append($("<option></option>").val(section.section_id).text(section.section_name)));
        });

jQuery(document).ready(function ($) {
      ...
      success: function (response) {
        if (response.success) {
          ...
          $editRow
            ...
            .html('<p style="color:red;">Could not load editor.</p>');
        }
      },
    });
  });
  ...
  wrapper.on("change", ".qe-subject-select", function () {
    ...
    var topicSelect = subjectSelect
      .closest(".form-row-flex")
      .find(".qe-topic-select");
    ...
    if (
      typeof qp_quick_edit_topics_data !== "undefined" &&
      selectedSubjectId &&
      qp_quick_edit_topics_data[selectedSubjectId]
    ) {
      ...
      $.each(
        ...
        function (index, topic) {
          ...
          if (
            typeof qp_current_topic_id !== "undefined" &&
            topic.id == qp_current_topic_id
          ) {
...
wrapper.on('click', '.save', function(e) {
    ...
    var questionStatus = $('#post-' + questionId).find('.editinline').data('status'); // Get status from original row link

    $button.prop('disabled', true).text('Updating...');

    $.ajax({
    url: ajaxurl, type: 'POST',
    data: $formWrapper.find(':input').serialize() + '&' + $.param({
      ...
      success: function (response) {
        if (response.success && response.data.row_html) {
          ...
          Swal.fire(
            "Error!",
            response.data.message || "Could not save changes.",
            "error"
          );
  ...
  $topicFilter.on("change", function () {
    ...
    var subjectId = $subjectFilter.val(); // Keep subjectId in case it's needed in the future
    $sourceFilter.hide().val("");

    // Only make the AJAX call if a specific topic is selected
    if (topicId) {
      $.ajax({
        url: qp_admin_filter_data.ajax_url,
        type: "POST",
        ...
        success: function (response) {
          if (
            ...
            $.each(response.data.sources, function (index, source) {
              // Add the main source as an option
              ...
              // Add its sections as indented options
  ...
  wrapper.on("change", ".qe-subject-select", function () {
    ...
    var availableExams = qp_quick_edit_data.all_exams
      .filter(function (exam) {
        return linkedExamIds.includes(exam.exam_id);
      })
      .map(function (exam) {
  ...
  wrapper.on("change", ".qe-source-select", function () {
    ...
    updateDropdown(
      $sectionSelect,
      qp_quick_edit_data.sections_by_source[sourceId],
  ...
  $(function () {
    ...
    var initialSubject = new URLSearchParams(window.location.search).get(
      "filter_by_subject"
    );
    ...
    function manageBulkEditPanel() {
      ...
      if (currentSubject && currentSubject === initialSubject) {
        ...
        var availableTopics = qp_bulk_edit_data.topics.filter(function (t) {
          return t.subject_id == currentSubject;
        });

        // Correctly filter available sources using the new link data
        var linkedSourceIds = qp_bulk_edit_data.source_subject_links
          .filter(function (link) {
            return link.subject_id == currentSubject;
          })
          .map(function (link) {
            return link.source_id;
          });
        var availableSources = qp_bulk_edit_data.sources.filter(function (
          source
        ) {
          return linkedSourceIds.includes(source.source_id);
        });

        var linkedExamIds = qp_bulk_edit_data.exam_subject_links
          .filter(function (l) {
            return l.subject_id == currentSubject;
          })
          .map(function (l) {
            return l.exam_id;
          });
        var availableExams = qp_bulk_edit_data.exams.filter(function (e) {
          return linkedExamIds.includes(e.exam_id);
        });
        ...
        $.each(availableTopics, function (i, topic) {
        ...
        $.each(availableSources, function (i, source) {
        ...
        $.each(availableExams, function (i, exam) {
          $examBulkEdit.append(
    ...
    $bulkEditPanel.on("change", "#bulk_edit_source", function () {
      ...
      var $sectionBulkEdit = $("#bulk_edit_section");
      var availableSections = qp_bulk_edit_data.sections.filter(function (sec) {
        return sec.source_id == selectedSource;
      });

      $sectionBulkEdit
        .html('<option value="">— Change Section —</option>')
        .prop("disabled", availableSections.length === 0);
      $.each(availableSections, function (i, section) {
      ...
      success: function (response) {
        if (response.success) {
          ...
          if (data.direction_text) {
            html +=
              '<div style="background: #f6f7f7; padding: 1rem; border-radius: 4px; margin-bottom: 1rem;">' +
              data.direction_text +
              "</div>";
          }
          html +=
            '<div class="question-text" style="margin-bottom: 1.5rem;">' +
            data.question_text +
            "</div>";
          ...
          data.options.forEach(function (opt) {
            var style =
              opt.is_correct == 1
                ...
                : "border-color: #e0e0e0;";
            html +=
              '<div class="option" style="border: 2px solid; padding: 1rem; margin-bottom: 0.5rem; border-radius: 8px; ' +
              style +
              '">' +
              opt.option_text +
              "</div>";

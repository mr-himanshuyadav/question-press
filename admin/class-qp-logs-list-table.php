...
class QP_Logs_List_Table extends WP_List_Table {
    ...
    public function get_columns() {
        return ['log_type' => 'Log Type', 'log_message' => 'Message', 'log_date' => 'Date'];
    }
    ...
    public function prepare_items() {
        ...
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name ORDER BY %s %s LIMIT %d OFFSET %d", $orderby, $order, $per_page, $offset), ARRAY_A);
        $this->set_pagination_args(['total_items' => $total_items, 'per_page' => $per_page]);
    }

    // UPDATED: Custom renderer for the message column to add review links
    public function column_log_message($item) {
        ...
        if (isset($log_data['question_id'])) {
            ...
            if ($group_id) {
                ...
                $message .= sprintf(' <a href="%s">Review Question &raquo;</a>', esc_url($review_link));
            }
        }
        return $message;
    }

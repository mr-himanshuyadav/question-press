jQuery(document).ready(function ($) {
  ...
  // --- Helper for Hierarchical Dropdowns ---
    function populateHierarchicalOptions($select, allTerms, parentId, depth = 0) {
        const children = allTerms.filter(t => t.parent == parentId);
        children.sort((a, b) => a.name.localeCompare(b.name));
        children.forEach(child => {
            const prefix = '-'.repeat(depth * 2) + (depth > 0 ? ' ' : '');
            $select.append(`<option value="${child.id}">${prefix}${child.name}</option>`);
            populateHierarchicalOptions($select, allTerms, child.id, depth + 1);
        });
    } //
  ...
  const modalHtml = `
    ...
    <div class="qp-modal-column-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
        <h3 style="margin:0;">Available Questions</h3>
        <label class="qp-bulk-select" style="font-size: 12px; cursor: pointer;">
            <input type="checkbox" id="qp-select-all-results"> Select All
        </label>
    </div>
...
<div id="qp-modal-selected-questions" class="qp-modal-column">
    <div class="qp-modal-column-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
        <h3 style="margin:0;">Selected (<span class="qp-selected-count">0</span>)</h3>
        <button type="button" id="qp-clear-selected-questions" class="button-link-delete" style="font-size:11px; text-decoration:none;">Clear All</button>
    </div>
  ...
  if (
    subjectDropdown.children("option").length <= 1 &&
    typeof qpCourseEditorData !== "undefined" &&
    qpCourseEditorData.testSeriesOptions &&
  ...
  $("#qp-modal-subject-filter")
    .off("change.qpsmodal")
    .on("change.qpsmodal", function () {
      ...
      $sourceSelect
        .empty()
        .prop("disabled", true)
        .append('<option value="">All Sources</option>');

      if (subjectId && typeof qpCourseEditorData !== "undefined") {
        // Populate Topics
        const topics =
          qpCourseEditorData.testSeriesOptions.allSubjectTerms.filter(
            (term) => term.parent == subjectId,
          );
        if (topics.length > 0) {
        ...
        // Populate Hierarchical Sources
            if (qpCourseEditorData.testSeriesOptions.sourceSubjectLinks && qpCourseEditorData.testSeriesOptions.allSourceTerms) {
                 ...
                 const roots = qpCourseEditorData.testSeriesOptions.allSourceTerms
                               .filter(term => linkedSourceIds.includes(String(term.id)));

                 if (roots.length > 0) {
                      $sourceSelect.prop('disabled', false);
                      roots.sort((a,b) => a.name.localeCompare(b.name)).forEach(root => {
                          $sourceSelect.append(`<option value="${root.id}">${root.name}</option>`);
                          populateHierarchicalOptions($sourceSelect, qpCourseEditorData.testSeriesOptions.allSourceTerms, root.id, 1);
                      });
                 }
            }
      }
  ...
  function reindexInputs() {
    sectionsList.find(".qp-section").each(function (sectionIndex, sectionEl) {
      ...
      $section
        .find("> .qp-section-header")
        .find(".qp-section-title-input")
        .attr("name", `${sectionBaseName}[title]`);
      $section
        .find('> input[name$="[section_id]"]')
        .attr("name", `${sectionBaseName}[section_id]`); // Re-index hidden section_id
      ...
      $section.find(".qp-course-item").each(function (itemIndex, itemEl) {
        ...
        $item
          .find(".qp-item-title-input")
          .attr("name", `${itemBaseName}[title]`);
        $item
          .find('input[name$="[item_id]"]')
          .attr("name", `${itemBaseName}[item_id]`); // Re-index hidden item_id
        $item
          .find(".qp-item-content-type-select")
          .attr("name", `${itemBaseName}[content_type]`);

        const contentType = $item.find(".qp-item-content-type-select").val();
        if (contentType === "test_series") {
          // Re-index all config inputs
          $item
            .find(".qp-test-series-config")
            .find("[data-config-key]")
            .each(function () {
              const $input = $(this);
          ...
          $item
            .find(".qp-selected-questions-input")
            .attr("name", `${itemBaseName}[config][selected_questions]`);
        }
  ...
  function renderItem(itemData, itemIndex, sectionIndex) {
    ...
    const scoringFieldsVisibility = isScoringEnabled
      ? "visibility: visible;"
      : "visibility: hidden;";
  ...
  function loadInitialStructure() {
    if (
      qpCourseEditorData &&
      qpCourseEditorData.structure &&
      qpCourseEditorData.structure.sections
    ) {
  ...
  sectionsList.on("click", ".qp-add-item-btn", function () {
    const $sectionContent = $(this)
      .closest(".qp-section-header")
      .next(".qp-section-content");
    ...
    reindexInputs();
    $itemsList
      .find(".qp-course-item:last-child .qp-config-subjects")
      .trigger("change");
    $itemsList
      .find(".qp-course-item:last-child .qp-config-scoring-enabled")
      .trigger("change");
  });
  ...
  sectionsList.on("change", ".qp-config-scoring-enabled", function () {
    ...
    const $marksGroup = $checkbox
      .closest(".qp-item-header-config")
      .find(".qp-marks-group");
    // --- END FIX ---
  ...
  structureContainer.on("click", ".qp-select-questions-btn", function () {
    ...
    currentModalTargetDisplay = $itemConfig.find(
      ".qp-selected-questions-display",
    );

    const currentIdsString = currentModalTargetInput.val();
    currentlySelectedInModal = currentIdsString
      ? currentIdsString
          .split(",")
    ...
    resultsList.html(
      '<p class="qp-modal-message">Use filters above to find questions.</p>',
    ); // Reset results
    $(".qp-pagination").empty();
  ...
  modal.on(
    "click",
    ".qp-modal-cancel-btn, .qp-modal-close-btn, .qp-modal-overlay",
    function (e) {
      if (
        $(e.target).hasClass("qp-modal-overlay") ||
        $(e.target).hasClass("qp-modal-cancel-btn") ||
        $(e.target).hasClass("qp-modal-close-btn")
      ) {
        modal.fadeOut(200);
  ...
  modal.on("click", ".qp-modal-done-btn", function () {
    if (currentModalTargetInput && currentModalTargetDisplay) {
      ...
      if (currentlySelectedInModal.length > 0) {
        currentModalTargetDisplay.html(
          `<span style="font-weight: bold;">${currentlySelectedInModal.length} questions selected:</span> ${selectedIdsString}`,
        );
      } else {
        currentModalTargetDisplay.html(
  ...
  function updateModalSelectedList() {
    ...
    if (currentlySelectedInModal.length === 0) {
      selectedList.html(
        '<p class="qp-modal-message" style="text-align: center; color: #777; padding: 10px;">Click "Add" on available questions to select them.</p>',
      );
      return;
    }
  ...
  // --- Function to update Add buttons (Hide if already selected) ---
    function updateResultsListButtons() {
        resultsList.find('.qp-result-item').each(function() {
            const $item = $(this);
            const qId = parseInt($item.data('id'), 10);
            const $button = $item.find('.qp-add-question-btn');

            if (currentlySelectedInModal.includes(qId)) {
                $button.hide(); // Hide button to prevent duplicates
            } else {
                $button.show(); // Ensure button is visible if not selected
            }
        });
    }
  ...
  modal.on("click", ".qp-remove-selected-btn", function () {
    ...
    currentlySelectedInModal = currentlySelectedInModal.filter(
      (id) => id !== qIdToRemove,
    );
    updateModalSelectedList();
  });
  ...
  function fetchQuestionsForModal(page = 1) {
    modalCurrentPage = page; // Update state
    if (currentAjaxRequest) {
      currentAjaxRequest.abort();
    }
    ...
    resultsList.html(
      '<p class="qp-modal-message"><i>Loading questions...</i></p>',
    );
    $(".qp-pagination").empty();
    ...
    currentAjaxRequest = $.ajax({
      ...
      data: {
        ...
        topic_id: topic,
        source_id: source,
        page: modalCurrentPage, // Pass page parameter
      },
      success: function (response) {
        ...
        if (
          ...
          response.data.questions.forEach((q) => {
            resultsList.append(`
        <div class="qp-result-item qp-animate-in" data-id="${q.id}">
            <div class="qp-result-info">
                <span class="qp-result-text">ID: ${q.id} - ${q.text}</span>
                <div class="qp-result-badges" style="margin-top: 4px; display: flex; gap: 5px;">
                    <span class="qp-badge" style="background: rgba(83, 0, 238, 0.05); padding: 2px 8px; border-radius: 999px; font-size: 10px; color: #5300ee; border: 1px solid rgba(83, 0, 238, 0.1); font-weight: 500;">
                        ${q.subject_badge}
                    </span>
                    ${
                      q.topic_badge
                        ? `
                    <span class="qp-badge" style="background: #f8fafc; padding: 2px 8px; border-radius: 999px; font-size: 10px; color: #64748b; border: 1px solid #e2e8f0; font-weight: 500;">
                        ${q.topic_badge}
                    </span>`
                        : ""
                    }
    ...
    `);
          ...
          renderModalPagination(response.data.total_pages, modalCurrentPage); // Handle pagination
          updateResultsListButtons();
        } else if (response.success) {
          resultsList.html(`
        <div class="qp-no-results">
            <span class="dashicons dashicons-search"></span>
            <p>No questions found. Try adjusting your intelligence filters.</p>
        </div>
    `);
        } else {
          resultsList.html(
            '<p class="qp-modal-message" style="color: red;">Error: ' +
              (response.data.message || "Could not fetch questions.") +
              "</p>",
          );
        }
      },
      error: function (jqXHR, textStatus) {
        if (textStatus !== "abort") {
          // Don't show error if we aborted it intentionally
          resultsList.html(
            '<p class="qp-modal-message" style="color: red;">An AJAX error occurred. Please try again.</p>',
          );
        }
      },
      complete: function () {
  ...
  function renderModalPagination(totalPages, currentPage) {
        const $container = $('.qp-pagination');
        $container.empty();
        console.log('Rendering pages:', totalPages);

        if (totalPages === 0) return; // Only hide if no results found at all

        let html = `<span class="pagination-info">Page ${currentPage} of ${totalPages}</span>`;
        html += `<div class="pagination-links" style="display: inline-block; margin-left: 10px;">`;
        if (currentPage > 1) {
            html += `<button type="button" class="button button-small qp-page-link" data-page="${currentPage - 1}">Prev</button>`;
        }
        if (currentPage < totalPages) {
            html += `<button type="button" class="button button-small qp-page-link" data-page="${currentPage + 1}">Next</button>`;
        }
        html += `</div>`;
        $container.html(html); //
    }

  // --- Real-time Listeners for Intelligence Panel ---
  const debouncedFetch = debounce(() => fetchQuestionsForModal(1), 300);

  modal.on("input", "#qp-modal-search-input", debouncedFetch); // Debounced search

  modal.on(
    "change",
    "#qp-modal-subject-filter, #qp-modal-topic-filter, #qp-modal-source-filter",
    function () {
      fetchQuestionsForModal(1); // Reset to page 1 on filter change
    },
  );

  modal.on("click", ".qp-page-link", function (e) {
    e.preventDefault();
    const page = parseInt($(this).data("page"), 10);
    fetchQuestionsForModal(page); // Trigger fetch for specific page
  });

  // Replace the manual filter button with the automatic logic or keep as backup
  modal.on("click", "#qp-modal-filter-btn", () => fetchQuestionsForModal(1));
  ...
  // --- Bulk Action: Select All in Search Results ---
  modal.on("change", "#qp-select-all-results", function () {
    const isChecked = $(this).is(":checked");
    resultsList.find(".qp-result-item").each(function () {
      const qId = parseInt($(this).data("id"), 10);
      if (isChecked && !currentlySelectedInModal.includes(qId)) {
        currentlySelectedInModal.push(qId);
      } else if (!isChecked) {
        currentlySelectedInModal = currentlySelectedInModal.filter(
          (id) => id !== qId,
  ...
  // --- Bulk Action: Clear All Selected ---
  modal.on("click", "#qp-clear-selected-questions", function () {
    if (confirm("Are you sure you want to clear all selected items?")) {
      currentlySelectedInModal = [];
      $("#qp-select-all-results").prop("checked", false);
      updateModalSelectedList();
    }
  });
  // ===========================================

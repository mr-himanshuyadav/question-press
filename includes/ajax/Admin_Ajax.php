...
class Admin_Ajax {
    ...
    /**
     * AJAX handler to update a product's price from the course edit meta box.
     */
    public static function update_product_price_from_course_meta() {
        // 1. Security Checks
        check_ajax_referer('qp_save_product_price_nonce', 'nonce');
        if ( ! current_user_can('edit_posts') ) { // Use a general capability
        ...
        $regular_price = isset($_POST['regular_price']) ? wc_format_decimal($_POST['regular_price']) : '';
        $sale_price = isset($_POST['sale_price']) ? wc_format_decimal($_POST['sale_price']) : '';

        if ( ! $product_id ) {
            wp_send_json_error(['message' => 'Invalid Product ID.'], 400);
        }

        // 4. Get Product and Update Prices
        ...
        // Check if this is our auto-generated product
        if ( $product->get_meta('_qp_is_auto_generated') !== 'true' ) {
             wp_send_json_error(['message' => 'This is not an auto-generated product.'], 403);
        }
        ...
        try {
            ...
            if ( $product_id ) {
                // Send back the sanitized prices to update the UI
                ...
                wp_send_json_error(['message' => 'Failed to save product price.'], 500);
            }

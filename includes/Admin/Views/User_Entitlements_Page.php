...
class User_Entitlements_Page
{
    ...
    {
        // 1. Get the search term (could be ID, username, or email)
        $search_term = isset($_GET['user_id']) ? sanitize_text_field($_GET['user_id']) : '';
        $user_info   = null;
        $user_id_searched = 0;

        if (! empty($search_term)) {
            // 2. Perform a flexible user search
            $user_query = new \WP_User_Query([
                'search'         => $search_term,
                'search_columns' => ['ID', 'user_login', 'user_email', 'display_name'],
                'number'         => 1, // We only want the most relevant match
            ]);
        ...
        $args = [
            ...
            'user_info'        => $user_info,
            'search_term'      => $search_term, // Optional: pass back to keep value in search box
        ];

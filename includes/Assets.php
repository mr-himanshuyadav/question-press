...
class Assets
{
    ...
    public function enqueue_admin_scripts($hook_suffix)
    {
        ...
        if ($hook_suffix === 'question-press_page_qp-settings') {
            wp_enqueue_style('qp-admin-update-css', QP_ASSETS_URL . 'css/admin-update.css', [], QP_PLUGIN_VERSION);
            wp_enqueue_script('qp-settings-script', QP_ASSETS_URL . 'js/settings-page.js', ['jquery', 'sweetalert2'], QP_PLUGIN_VERSION, true);

            wp_localize_script('qp-settings-script', 'qp_admin_object', [
                'ajax_url' => admin_url('admin-ajax.php'),
                'nonce'    => wp_create_nonce('qp_admin_nonce')
            ]);
        }

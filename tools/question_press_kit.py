...
class App(tk.Tk):
    def __init__(self):
        ...
        notebook.add(manual_entry_frame, text="Manual Question Entry")
    ...
    def create_manual_entry_widgets(self, parent_frame):
        ...
        self.direction_text.grid(row=1, column=1, columnspan=3, sticky="ew", padx=5, pady=5)
        ttk.Label(group_frame, text="Direction Image:").grid(row=2, column=0, sticky="w", padx=5, pady=5)
        ttk.Button(group_frame, text="Upload Image...", command=self.upload_dir_image).grid(row=2, column=1, sticky="w", padx=5, pady=5)
        ...
        ttk.Checkbutton(question_frame, text="Is PYQ?", variable=self.is_pyq_var).grid(row=1, column=1, sticky="w", padx=5, pady=5)
        question_frame.columnconfigure(1, weight=1)
        options_frame = ttk.LabelFrame(question_frame, text="Options (Mark the correct one)", padding=(10, 5))
        options_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=5, pady=5)
        self.correct_option_var = tk.IntVar(value=1)
        self.option_vars = []
        for i in range(5):
            ttk.Radiobutton(options_frame, text=f"Opt {i+1}", variable=self.correct_option_var, value=i+1).grid(row=i, column=0, sticky="w", padx=5, pady=2)
        ...
        ttk.Button(group_manage_frame, text="Generate ZIP File...", command=self.generate_zip).pack(side="right", padx=5)
    ...
    def upload_dir_image(self):
        ...
        self.current_group_image_path = filepath
        try:
    ...
    def add_question_to_group(self):
        ...
        if not question or len(options) < 2:
            messagebox.showerror("Error", "Question text and at least two options are required.")
            return
    ...
    def finish_group(self):
        ...
        if not subject: messagebox.showerror("Error", "Subject cannot be empty."); return
        if not self.current_group_questions: messagebox.showerror("Error", "Cannot finish an empty group."); return
        
        group_data = {
            "subject": subject, "direction_text": self.direction_text.get("1.0", "end-1c").strip(),
            "image_path": self.current_group_image_path, "questions": self.current_group_questions
        }
        self.question_groups.append(group_data)
        self.current_group_questions, self.current_group_image_path = [], None
        self.image_preview_label.config(image='', text="No image selected.")
        self.direction_text.delete("1.0", "end")
        self.group_status_var.set(f"{len(self.question_groups)} question groups finalized.")
    ...
    def generate_zip(self):
        ...
        for group in self.question_groups:
            ...
            if group["image_path"]:
                try:
                    image_filename = os.path.basename(group["image_path"])
                    shutil.copy(group["image_path"], os.path.join(images_dir, image_filename))
                except Exception as e:
        ...
        final_json_data = {
            "schemaVersion": "1.2", "exportTimestamp": datetime.utcnow().isoformat() + "Z",
            "sourceFile": os.path.basename(filepath), "questionGroups": final_question_groups
        }
        json_path = os.path.join(temp_dir, "questions.json")
        with open(json_path, 'w', encoding='utf-8') as f: json.dump(final_json_data, f, indent=2)
